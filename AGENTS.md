...
- 지역 meta는 오너가 마사지봄에서 승인한 8종을 이 순서로 고정한다: `<표시지역>출장마사지`, `<표시지역>출장안마`, `<표시지역>출장타이마사지`, `<표시지역>출장스웨디시`, `<표시지역>출장홈타이`, `<표시지역>토닥이`, `<표시지역>남성전용마사지`, `<표시지역>여성전용마사지`. title은 앞의 두 핵심어와 현재 플랫폼 브랜드를 사용하고, description은 두 핵심어·코스/이용시간·24시간 전화·선입금 없는 현장결제를 설명한다. `지역 안내`, route kind, `현장후불`을 8종 대신 임의 meta keyword로 넣지 않는다.
